...
getCTGeos <- function(level = "town") {
    ...
    for(i in 1:length(countyfips)) {
        ...
        if (returnTowns) { # If we're return towns (or both)
            subdivs <- as.numeric(subdiv_fips[subdiv_fips$county==county,4][[1]])
...
getACSData <- function(geoObject, yearList, get_this_one) {
    data.list <- list()
    for (y in yearList) {
        print(paste("Processing ACS year", y, sep=":"))
        # check if file exists for given year/table
        filename <- paste("ACS", (as.numeric(y)-2000), "5YR", get_this_one, "with", "ann.csv", sep = "_")
